...
st.markdown('<div style="text-align: center; color: #666;">照顧路上，您辛苦了！跟著好厝邊簡單評估長照 3.0 資格。</div>', unsafe_allow_html=True)

# ---------------------------------------------------------
# 2. 更新後的 10 項溫馨題目 (分三段式選項)
# ---------------------------------------------------------
questions = {
    "🏠 平常在家動一動": [
    ...
    "🚌 出門走走與生活": [
        ...
        {"id": "b5", "label": "10. 處理財務", "options": ["去銀行或超商算錢、繳費都沒問題", "小錢還可以，大錢（如去銀行）會糊塗", "現在完全不能讓他管錢了"]},
    ]
}
...
with col1:
    is_aboriginal = st.checkbox("具有原住民身分 (55歲以上即符合)")
...
def calculate_3_0_logic(responses, is_pac_status):
    ...
    for q_id, val in responses.items():
        ...
        score += (choice_idx * 1.5) # 加重係數

    # 將總分轉化為機率 (Sigmoid 函數模擬)
    z = -4.5 + score
...
if st.button("✨ 點我開始評估"):
    if placeholder in user_responses.values():
        st.error("⚠️ 還有題目漏掉囉！請把 10 個題目都選好再點我。")
    else:
        is_match, prob = calculate_3_0_logic(user_responses, is_pac)
        
        st.markdown(f"""
        <div class="result-box">
            <div style='color: #666; font-size: 1.1rem;'>推估政府補助媒合度</div>
        ...
        if is_match and prob >= 0.4:
            ...
            st.markdown("""
                * **身分狀態：** 符合政府收案族群。
                * **失能狀況：** 評估已達 CMS 2級以上標準。
                * **建議行動：** 準備好身分證字號與地址，撥打 **1966** 申請。
            """)
            st.balloons()
        elif is_match and prob < 0.4:
            st.warning("🟡 **身分符合，但目前失能等級較輕。**")
            st.write("雖然您符合收案身分，但目前長輩自理能力不錯。若之後有退化現象，請隨時回來重測。")
        else:
            st.info("⚪ **目前尚未符合補助門檻。**")
            st.write("若未來年齡達標或診斷出失智/身障證明，即可符合申請資格。")
